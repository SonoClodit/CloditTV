...
final extensionController = ExtensionController();
...
class ExtensionManifest {
  ...
  factory ExtensionManifest.fromJson(Map<String, dynamic> json) {
    ...
    return ExtensionManifest(
      ...
      language: json['language'] as String? ?? 'MULTI',
      version: json['version'] as String? ?? '1.0.0',
      icon: _extensionIcon(json['icon'] as String?),
      capabilities: capabilitiesJson is List
          ? capabilitiesJson.map((item) => item.toString()).toList()
          : const [],
...
IconData _extensionIcon(String? name) {
  switch (name) {
    case 'movies':
...
const cloditExtensionRepository = <ExtensionManifest>[
  ExtensionManifest(
    id: 'org.clodit.tmdb',
    name: 'TMDB',
    description: 'Catalogo, ricerca, locandine e informazioni in italiano.',
    language: 'IT',
    version: '1.0.0',
    icon: Icons.local_movies_rounded,
    capabilities: ['Catalogo', 'Ricerca', 'Dettagli'],
    system: true,
  ),
  ExtensionManifest(
    id: 'org.clodit.archive',
    name: 'Internet Archive',
    description: 'Film e video di pubblico dominio riproducibili legalmente.',
    language: 'MULTI',
    version: '1.0.0',
    icon: Icons.account_balance_rounded,
    capabilities: ['Catalogo', 'Ricerca', 'Riproduzione'],
  ),
  ExtensionManifest(
    id: 'org.clodit.library',
    name: 'Libreria personale',
    description: 'Organizza e riproduce i file video posseduti dall’utente.',
    language: 'LOCALE',
    version: '1.0.0',
    icon: Icons.video_library_rounded,
    capabilities: ['File locali', 'Riproduzione'],
  ),
  ExtensionManifest(
    id: 'org.clodit.demo.it',
    name: 'Demo italiano',
    description: 'Provider didattico per testare catalogo, episodi e sorgenti.',
    language: 'IT',
    version: '1.0.0',
    icon: Icons.science_rounded,
    capabilities: ['Catalogo', 'Episodi', 'Sorgenti demo'],
  ),
];

class ExtensionController extends ChangeNotifier {
  static const repositoryUrl =
      'https://raw.githubusercontent.com/SonoClodit/CloditTV/main/extensions/index.json';

  Set<String> installed = {'org.clodit.tmdb'};
  List<ExtensionManifest> repository =
      List<ExtensionManifest>.from(cloditExtensionRepository);
  ...
  Future<void> load() async {
    ...
    installed = {
      'org.clodit.tmdb',
      ...?prefs.getStringList('clodit_extensions'),
    };
  ...
  Future<void> refreshRepository() async {
    ...
    try {
      final response = await http
          .get(Uri.parse(repositoryUrl))
          .timeout(const Duration(seconds: 8));

      if (response.statusCode != 200) {
        throw Exception('Errore repository: ${response.statusCode}');
      }

      final decoded = jsonDecode(response.body);
      if (decoded is! Map<String, dynamic>) {
        throw const FormatException('Formato repository non valido');
      }

      final extensionsJson = decoded['extensions'];
      if (extensionsJson is! List) {
        throw const FormatException('Elenco estensioni non valido');
      }

      final downloaded = extensionsJson
          .map(
            (item) => ExtensionManifest.fromJson(
      ...
      if (downloaded.isEmpty) {
        throw const FormatException('Il repository è vuoto');
      }

      repository = downloaded;
    } catch (exception) {
      repositoryError = exception.toString();
  ...
  Future<void> toggle(ExtensionManifest extension) async {
    if (extension.system) return;
    ...
    await prefs.setStringList(
      'clodit_extensions',
      installed.where((id) => id != 'org.clodit.tmdb').toList(),
    );
  }
}
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                AnimatedBuilder(
                  animation: extensionController,
                  builder: (context, _) => ListTile(
                    leading: const Icon(Icons.extension_rounded),
                    title: const Text('Estensioni'),
                    subtitle: Text(
                      '${extensionController.installed.length} installate',
                    ),
        ...
        actions: [
          AnimatedBuilder(
            ...
            builder: (context, _) => IconButton(
              tooltip: 'Aggiorna repository',
              onPressed: extensionController.refreshing
                  ? null
                  : extensionController.refreshRepository,
              icon: extensionController.refreshing
                  ? const SizedBox(
                      width: 20,
                      height: 20,
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) => ListView(
          ...
          children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 3),
                        Text('Estensioni verificate e compatibili con iOS.'),
                      ],
                    ),
                  ),
                ],
            ...
            ...extensionController.repository.map((extension) {
              final installed = extensionController.isInstalled(extension.id);
              return Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                        ...
                        child: Icon(extension.icon, color: colors.primary),
                      ),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Text(
                                  '${extension.language} · v${extension.version}',
                            ...
                            Wrap(
                              spacing: 6,
                              runSpacing: 6,
                              children: extension.capabilities
                                  .map((item) => Chip(
                                        label: Text(item),
                            ...
                            Align(
                              ...
                              child: extension.system
                                  ...
                                  : FilledButton.tonal(
                                      onPressed: () =>
                                          extensionController.toggle(extension),
                                      child: Text(
                                        installed ? 'Disinstalla' : 'Installa',
                                      ),
                                    ),
                            ),
                          ],
